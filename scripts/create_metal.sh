...
export metal_machineset_name=$(echo $ref_machineset_name | sed s/worker/metal/)

# Replace machine replicas number
export NODE_NO=$(echo "${1:-2}")

# Desired GPU instance type
export instance_type=$(echo "${2:-m5zn.metal}")	
...
oc get -n openshift-machine-api machineset $ref_machineset_name -o json \
    ...
    | jq --arg metal_machineset_name "${metal_machineset_name}" '.metadata.name = $metal_machineset_name' \
    | jq --arg metal_machineset_name "${metal_machineset_name}" '.spec.template.metadata.labels."machine.openshift.io/cluster-api-machineset" = $metal_machineset_name' \
    | jq --arg metal_machineset_name "${metal_machineset_name}" '.spec.selector.matchLabels."machine.openshift.io/cluster-api-machineset" = $metal_machineset_name' \
